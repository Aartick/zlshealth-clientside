...
function Page() {
    ...
    // Store selecte image
    const [selectedImage, setSelectedImage] = useState(product.productImg.url)
    ...
    const getDetails = async () => {
        try {
            ...
            setSelectedImage(details.data.result.productImg.url)
        } catch { }
        finally {
            setLoading(false)
        }
    ...
    return (
        <div className='space-y-5 pb-10 max-w-screen-2xl mx-auto'>
            {(product._id === "" && !loading) ? (
                <p className="flex justify-center items-center text-[#919191] text-xl h-1/2">No such product found</p>
            ) : (
                <>
                    ...
                    ${similarProducts.length === 0 && "justify-center"}
                    `}
                        >
                            {similarProducts.length === 0
                                ? <p className="text-center text-[#919191]">No products found</p>
